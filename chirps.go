...
func (cfg *apiConfig) handlerDeleteChirpByID(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		log.Printf("error retrieving chirp by id: %v", err)
		return
	}

	if chirp.UserID != userID {
		respondWithError(w, http.StatusForbidden, "Unauthorized deletion")
		return
	}

	err = cfg.db.DeleteChirpByID(r.Context(), uid)
	if err != nil {
...
func (cfg *apiConfig) handlerGetAllChirps(w http.ResponseWriter, r *http.Request) {
	...
	if author_id != "" {
		...
		chirps, err = cfg.db.GetAllChirpsByUserID(r.Context(), uid)
		if err != nil {
			respondWithError(w, http.StatusInternalServerError, "error retrieving chirps")
			log.Printf("error retrieving chirps by userID: %s", err)
			return
		}
	} else {
